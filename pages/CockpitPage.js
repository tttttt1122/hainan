window.CockpitPage = class CockpitPage {
                        ...
                        <div id="tab-home" class="cockpit-tab-pane active" style="display: grid;">
                            <div class="cockpit-row">
                                <div class="card-section right-section">
                                    <div class="card-title-left">预警投诉</div>
                                    ...
                                    <div class="behavior-warning-grid">
                                        <div class="behavior-warning-item cp-clickable" data-cp-modal="66">
                                            <span class="behavior-warning-label">事项未覆盖</span>
                                            <span class="behavior-warning-value" style="color: #007aff;">0</span>
                                        </div>
                                        <div class="behavior-warning-item cp-clickable" data-cp-modal="68">
                                            <span class="behavior-warning-label">事项未关联</span>
                                            <span class="behavior-warning-value" style="color: #007aff;">0</span>
                                        </div>
                                        ...
                                        <div class="behavior-warning-item cp-clickable" data-cp-modal="63">
                                            ...
                                            <span class="behavior-warning-value" style="color: #ff3b30;">4</span>
                                        </div>
                                        <div class="behavior-warning-item cp-clickable" data-cp-modal="64">
                                            <span class="behavior-warning-label">联合惩戒应执行未执行主体数量</span>
                                            <span class="behavior-warning-value" style="color: #ff3b30;">6</span>
                                        </div>
                                    ...
                                    <div class="map-container">
                                        ...
                                        <div class="risk-signal-overlay">
                                            ...
                                            <div class="risk-item">
                                                ...
                                                <span class="risk-value">事项未覆盖</span>
                                            </div>
                                ...
                                <div class="card-section supervision-section">
                                    <div class="card-title">预警分析</div>
                                    <div class="trend-chart-title-left">近期发现预警趋势</div>
                                    <div id="supervision-trend-chart" class="chart-container" style="height: 140px; width: 100%;"></div>
                                    <div class="trend-chart-title-left">累计预警最多的模型TOP10</div>
                                    <div id="supervision-bar-chart" class="chart-container" style="height: 240px; width: 100%;"></div>
                                </div>
                            </div>
                                ...
                                <div class="card-section dispatch-card-home">
                                    ...
                                    <div class="dispatch-content-row">
                                        <div class="dispatch-item cp-clickable" data-cp-modal="8">
                                            ...
                                            <span class="dispatch-value-sm">99</span>
                                        </div>
    ...
    initHomeCharts() {
        ...
        const trendEl = document.getElementById('supervision-trend-chart');
        if (trendEl) {
            if (this.charts['supervision-trend-chart']) {
                this.charts['supervision-trend-chart'].dispose();
            }
            const trendChart = echarts.init(trendEl);
            trendChart.setOption({
                textStyle: { color: 'rgba(255,255,255,0.6)' },
                tooltip: { trigger: 'axis', formatter: '{b}: {c}条' },
                grid: { left: '10%', right: '8%', bottom: '12%', top: '5%', containLabel: true },
                xAxis: {
                    type: 'category',
                    data: ['07-11', '07-12', '07-13', '07-14', '07-15', '07-16', '07-17', '07-18', '07-19', '07-20'],
                ...
                series: [{
                    name: '预警信息数',
                    type: 'line',
                    data: [12, 8, 15, 22, 18, 14, 25, 19, 16, 20],
                    smooth: true,
                    symbol: 'circle',
                    symbolSize: 6,
            ...
            this.charts['supervision-trend-chart'] = trendChart;
        }

        const barEl = document.getElementById('supervision-bar-chart');
        if (barEl) {
            if (this.charts['supervision-bar-chart']) {
                this.charts['supervision-bar-chart'].dispose();
            }
            const barChart = echarts.init(barEl);
            barChart.setOption({
                textStyle: { color: 'rgba(255,255,255,0.6)' },
                tooltip: { trigger: 'axis', formatter: '{b}: {c}条' },
                grid: { left: '10%', right: '8%', bottom: '6%', top: '6%', containLabel: true },
                xAxis: {
                    type: 'value',
                    axisLine: { lineStyle: { color: 'rgba(0,212,255,0.2)' } },
                ...
                yAxis: {
                    type: 'category',
                    data: ['违规处罚撤销', '超出裁量标准', '信用修复超期', '处罚公示超期', '失信信息公示滞后', '处罚办案超期', '审批材料补正', '审批超承诺期限', '线下线索流转工单', '线索材料退回'],
                    axisLine: { lineStyle: { color: 'rgba(0,212,255,0.2)' } },
                    axisLabel: { color: 'rgba(255,255,255,0.8)', fontSize: 10, interval: 0 },
                    splitLine: { show: false }
                },
                series: [{
                    name: '预警信号数',
                    type: 'bar',
                    data: [1, 2, 2, 3, 4, 5, 8, 12, 56, 84],
                    barWidth: '55%',
                    itemStyle: {
                    ...
                    label: { show: true, position: 'right', fontSize: 10, color: 'rgba(255,255,255,0.8)' }
                }]
            });
            this.charts['supervision-bar-chart'] = barChart;
        }
    }
    ...
    _cpTitle(m) {
        const map = {
            ...
            65:'信用修复超期办理预警分析',
            66:'事项覆盖率全景分析', 67:'处罚发生率深度分析', 68:'事项关联率全景分析', 72:'审批办件无依据不受理预警分析'
        };
        return map[m] || '';
    }
    ...
    switchTab(tab) {
        ...
        // 更新 header 中的子选卡高亮
        const headerSubTabs = document.querySelectorAll('.cockpit-sub-tab');
        headerSubTabs.forEach(item => item.classList.remove('active'));
        const activeSubTab = document.querySelector(`.cockpit-sub-tab[data-cockpit-tab="${tab}"]`);
    ...
    // ============ MODAL 72: 审批办件无依据不受理预警分析 ============
    genCp72Data() {
        const domains = ['市场监管', '卫生健康', '交通运输', '生态环境'];
        const departments = ['市场监管局', '卫生健康委', '交通运输局', '生态环境局'];
        const matters = ['食品经营许可', '医疗机构执业许可', '道路运输经营许可', '环境影响评价审批'];
        const applicants = ['海南鑫达食品有限公司', '三亚康泰医院', '海南顺通运输公司', '海南绿能环保科技'];
        return Array.from({ length: 15 }, (_, i) => ({
            id: i + 1,
            code: `WD${String(i + 1).padStart(6, '0')}`,
            matter: matters[i % 4],
            applicant: applicants[i % 4],
            domain: domains[i % 4],
            department: departments[i % 4],
            date: `2026-07-${String(10 - (i % 10)).padStart(2, '0')}`
        }));
    }

    openCpModal72() { this._cpOpen(72, this.genCp72Data()); }
    ...
    _cpHtml72() {
        ...
        return `
            <div class="rv-modal" style="width: 85vw;">
                <div class="rv-modal-header"><span class="rv-modal-title">${this._cpTitle(72)}</span><span class="rv-modal-close">×</span></div>
                <div class="rv-modal-body">
                    <div class="rv-stat-row">
                        ${this._cpRenderCard('无依据不受理工单总数', '3件')}
                        ${this._cpRenderCard('涉及审批事项', '3项')}
                        ${this._cpRenderCard('涉及部门', '2个')}
                    </div>
                    <div class="rv-charts">
                        <div class="rv-chart-left" style="flex:5">
                            <div class="rv-chart-item">
                                <span class="rv-chart-title">无依据不受理事项类型分布</span>
                                <div id="cpc72" class="rv-chart"></div>
                            </div>
                        </div>
                        <div class="rv-chart-right" style="flex:5">
                            <div class="rv-chart-item">
                                <span class="rv-chart-title">各领域无依据不受理工单分布</span>
                                <div id="cpc72b" class="rv-chart"></div>
                            </div>
                        </div>
                    </div>
                    <div class="rv-table-wrap">
                        ...
                        <div class="rv-pagination">
                            <span class="rv-page-prev ${this.cpCurrentPage <= 1 ? 'disabled' : ''}">‹</span>
                            <span class="rv-page-info">${this.cpCurrentPage}/${totalPages}</span>
                            <span class="rv-page-next ${this.cpCurrentPage >= totalPages ? 'disabled' : ''}">›</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    _cpChart72() {
        const pieChart = echarts.init(document.getElementById('cpc72'));
        pieChart.setOption({
            tooltip: { trigger: 'item', formatter: '{b}: {c}万项' },
            legend: { bottom: 0, textStyle: { color: 'rgba(255,255,255,0.5)', fontSize: 10 } },
            series: [{
                type: 'pie',
                radius: ['45%', '70%'],
                center: ['50%', '45%'],
                label: { show: true, fontSize: 10, color: 'rgba(255,255,255,0.7)', formatter: '{b}: {c}万项' },
                data: [
                    { value: 1.8, name: '行政许可', itemStyle: { color: '#00d4ff' } },
                    { value: 1.1, name: '行政确认', itemStyle: { color: '#34c759' } },
                    { value: 2, name: '行政裁决', itemStyle: { color: '#ff9500' } },
                    { value: 0.9, name: '行政奖励', itemStyle: { color: '#ff3b30' } },
                    { value: 1.5, name: '行政给付', itemStyle: { color: '#af52de' } },
                    { value: 1, name: '公共服务', itemStyle: { color: '#5856d6' } },
                    { value: 2, name: '其他行政权力', itemStyle: { color: '#ffd54f' } }
                ]
            }]
        });

        const barChart = echarts.init(document.getElementById('cpc72b'));
        barChart.setOption({
            tooltip: { trigger: 'axis', formatter: '{b}: {c}件' },
            grid: { left: '3%', right: '4%', bottom: '3%', top: '10%', containLabel: true },
            xAxis: { 
                type: 'category', 
                data: ['市场监管', '卫生健康', '交通运输', '生态环境'], 
                axisLabel: { color: 'rgba(255,255,255,0.5)', fontSize: 10 } 
            },
            yAxis: { 
                type: 'value', 
                axisLabel: { color: 'rgba(255,255,255,0.5)', fontSize: 10, formatter: '{value}件' },
                splitLine: { lineStyle: { color: 'rgba(0,212,255,0.1)' } }
            },
            series: [{
                type: 'bar',
