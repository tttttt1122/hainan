window.CreditPage = class CreditPage {
    constructor(container) {
        ...
        this.charts = {};
        // 行政许可弹窗
        this.xkComplianceData = []; this.xkCompPage = 1; this.xkCompPageSize = 10; this._xkCompOvl = null; this._xkCompCharts = {};
        this.xkLateData = []; this.xkLatePage = 1; this.xkLatePageSize = 10; this._xkLateOvl = null; this._xkLateCharts = {};
        this.xkConcealData = []; this.xkConcPage = 1; this.xkConcPageSize = 10; this._xkConcOvl = null; this._xkConcCharts = {};
        // 行政处罚弹窗
        this.xzComplianceData = []; this.xzCompPage = 1; this.xzCompPageSize = 10; this._xzCompOvl = null; this._xzCompCharts = {};
        this.xzLateData = []; this.xzLatePage = 1; this.xzLatePageSize = 10; this._xzLateOvl = null; this._xzLateCharts = {};
        this.xzConcealData = []; this.xzConcPage = 1; this.xzConcPageSize = 10; this._xzConcOvl = null; this._xzConcCharts = {};
        // 信用承诺弹窗
        this.promiseApplyData = []; this.paPage = 1; this.paPageSize = 10; this._paOvl = null; this._paCharts = {};
        this.promiseHandleData = []; this.phPage = 1; this.phPageSize = 10; this._phOvl = null; this._phCharts = {};
        this.promiseNaturalData = []; this.pnPage = 1; this.pnPageSize = 10; this._pnOvl = null; this._pnCharts = {};
        this.promiseLegalData = []; this.plPage = 1; this.plPageSize = 10; this._plOvl = null; this._plCharts = {};
        // 联合奖惩弹窗
        this.rewardRedData = []; this.rrPage = 1; this.rrPageSize = 10; this._rrOvl = null; this._rrCharts = {};
        this.rewardBlackData = []; this.rbPage = 1; this.rbPageSize = 10; this._rbOvl = null; this._rbCharts = {};
        // 失信情况弹窗
        this.dishonestGeneralData = []; this.dgPage = 1; this.dgPageSize = 10; this._dgOvl = null; this._dgCharts = {};
        this.dishonestSeriousData = []; this.dsPage = 1; this.dsPageSize = 10; this._dsOvl = null; this._dsCharts = {};
        this.render();
        this.initTabs();
        this.initCharts();
        this.bindEvents();
    ...
    renderXkLateModal(){const data=this.xkLateData;const tp=Math.ceil(data.length/this.xkLatePageSize);const pd=data.slice((this.xkLatePage-1)*this.xkLatePageSize,this.xkLatePage*this.xkLatePageSize);return`<div class="rv-modal"><div class="rv-modal-header xkl-header"><span class="rv-modal-title">行政许可数据迟报分析</span><button class="rv-modal-close">×</button></div><div class="rv-stats"><div class="rv-stat-card"><span class="rv-stat-label">数据迟报率</span><span class="rv-stat-value" style="color:#ff9500">2%</span></div><div class="rv-stat-card"><span class="rv-stat-label">迟报数据量</span><span class="rv-stat-value" style="color:#ff3b30">98条</span></div><div class="rv-stat-card"><span class="rv-stat-label">按时报送数据量</span><span class="rv-stat-value" style="color:#34c759">4,802条</span></div></div><div class="rv-charts"><div class="rv-chart-left" style="flex:5"><div class="rv-chart-item"><span class="rv-chart-title">近6个月迟报率趋势</span><div id="xkl-line" class="rv-chart-container"></div></div></div><div class="rv-chart-right" style="flex:5"><div class="rv-chart-item"><span class="rv-chart-title">迟报部门分布TOP5</span><div id="xkl-pie" class="rv-chart-container"></div></div></div></div><div class="rv-table-wrap"><table class="rv-table"><thead><tr><th>序号</th><th>数据编号</th><th>业务类型</th><th>报送部门</th><th>应报时间</th><th>实报时间</th><th>延迟天数</th></tr></thead><tbody>${pd.map(r=>`<tr><td>${r.seq}</td><td>${r.code}</td><td>${r.bizType}</td><td>${r.dept}</td><td>${r.dueTime}</td><td>${r.realTime}</td><td style="color:#ff9500">${r.delayDays}天</td></tr>`).join('')}</tbody></table></div><div class="rv-footer"><button class="rv-page-btn" id="xkl-prev" ${this.xkLatePage<=1?'disabled':''}>上一页</button><span class="rv-page-info">第 ${this.xkLatePage} / ${tp} 页</span><button class="rv-page-btn" id="xkl-next" ${this.xkLatePage>=tp?'disabled':''}>下一页</button></div></div>`;}
    ...
    initXkLateCharts(){if(typeof echarts==='undefined')return;requestAnimationFrame(()=>requestAnimationFrame(()=>setTimeout(()=>{this.initXklLine();this.initXklPie();},150)));}
    ...
    initXklPie(){const dom=this._xkLateOvl?.querySelector('#xkl-pie');if(!dom)return;dom.style.cssText='width:100%;height:230px';if(dom.getBoundingClientRect().width<=0){setTimeout(()=>this.initXklPie(),200);return;}const ex=echarts.getInstanceByDom(dom);if(ex)ex.dispose();const c=echarts.init(dom);this._xkLateCharts.pie=c;c.setOption({tooltip:{trigger:'item',formatter:'{b}: {c}次 ({d}%)'},legend:{bottom:0,textStyle:{color:'rgba(255,255,255,0.6)',fontSize:11}},series:[{type:'pie',radius:['50%','72%'],center:['50%','45%'],itemStyle:{borderRadius:6,borderColor:'rgba(5,13,24,0.98)',borderWidth:3},label:{formatter:'{b}\n{d}%',color:'rgba(255,255,255,0.7)',fontSize:10},data:[{value:28,name:'市场监管局',itemStyle:{color:'#ff9500'}},{value:22,name:'综合执法局',itemStyle:{color:'#ff6b3d'}},{value:18,name:'生态环境局',itemStyle:{color:'#ff3b30'}},{value:15,name:'交通运输局',itemStyle:{color:'#af52de'}},{value:17,name:'其他',itemStyle:{color:'#8e8e93'}}]}]});new ResizeObserver(()=>c.resize()).observe(dom);}
    ...
    renderXzLateModal(){const data=this.xzLateData;const tp=Math.ceil(data.length/this.xzLatePageSize);const pd=data.slice((this.xzLatePage-1)*this.xzLatePageSize,this.xzLatePage*this.xzLatePageSize);return`<div class="rv-modal"><div class="rv-modal-header xzl-header"><span class="rv-modal-title">行政处罚数据迟报分析</span><button class="rv-modal-close">×</button></div><div class="rv-stats"><div class="rv-stat-card"><span class="rv-stat-label">数据迟报率</span><span class="rv-stat-value" style="color:#ff9500">3%</span></div><div class="rv-stat-card"><span class="rv-stat-label">迟报数据量</span><span class="rv-stat-value" style="color:#ff3b30">66条</span></div><div class="rv-stat-card"><span class="rv-stat-label">按时报送数据量</span><span class="rv-stat-value" style="color:#34c759">2,134条</span></div></div><div class="rv-charts"><div class="rv-chart-left" style="flex:5"><div class="rv-chart-item"><span class="rv-chart-title">近6个月迟报率趋势</span><div id="xzl-line" class="rv-chart-container"></div></div></div><div class="rv-chart-right" style="flex:5"><div class="rv-chart-item"><span class="rv-chart-title">迟报部门分布TOP5</span><div id="xzl-pie" class="rv-chart-container"></div></div></div></div><div class="rv-table-wrap"><table class="rv-table"><thead><tr><th>序号</th><th>数据编号</th><th>处罚类型</th><th>报送部门</th><th>应报时间</th><th>实报时间</th><th>延迟天数</th></tr></thead><tbody>${pd.map(r=>`<tr><td>${r.seq}</td><td>${r.code}</td><td>${r.penaltyType}</td><td>${r.dept}</td><td>${r.dueTime}</td><td>${r.realTime}</td><td style="color:#ff9500">${r.delayDays}天</td></tr>`).join('')}</tbody></table></div><div class="rv-footer"><button class="rv-page-btn" id="xzl-prev" ${this.xzLatePage<=1?'disabled':''}>上一页</button><span class="rv-page-info">第 ${this.xzLatePage} / ${tp} 页</span><button class="rv-page-btn" id="xzl-next" ${this.xzLatePage>=tp?'disabled':''}>下一页</button></div></div>`;}
    ...
    initXzLateCharts(){if(typeof echarts==='undefined')return;requestAnimationFrame(()=>requestAnimationFrame(()=>setTimeout(()=>{this.initXzlLine();this.initXzlPie();},150)));}
    ...
    initXzlPie(){const dom=this._xzLateOvl?.querySelector('#xzl-pie');if(!dom)return;dom.style.cssText='width:100%;height:230px';if(dom.getBoundingClientRect().width<=0){setTimeout(()=>this.initXzlPie(),200);return;}const ex=echarts.getInstanceByDom(dom);if(ex)ex.dispose();const c=echarts.init(dom);this._xzLateCharts.pie=c;c.setOption({tooltip:{trigger:'item',formatter:'{b}: {c}次 ({d}%)'},legend:{bottom:0,textStyle:{color:'rgba(255,255,255,0.6)',fontSize:11}},series:[{type:'pie',radius:['50%','72%'],center:['50%','45%'],itemStyle:{borderRadius:6,borderColor:'rgba(5,13,24,0.98)',borderWidth:3},label:{formatter:'{b}\n{d}%',color:'rgba(255,255,255,0.7)',fontSize:10},data:[{value:24,name:'综合执法局',itemStyle:{color:'#ff9500'}},{value:18,name:'市场监管局',itemStyle:{color:'#ff6b3d'}},{value:12,name:'交通运输局',itemStyle:{color:'#ff3b30'}},{value:8,name:'生态环境局',itemStyle:{color:'#af52de'}},{value:14,name:'其他',itemStyle:{color:'#8e8e93'}}]}]});new ResizeObserver(()=>c.resize()).observe(dom);}
    ...
    // 弹窗十：信用承诺 - 法人违诺(966)
    // ============================================
    genPromiseLegalData(){const d=[];const types=['未履行承诺','虚假承诺','逾期履行','其他'];const names=['XX建筑集团有限公司','YY餐饮连锁有限公司','ZZ零售商贸有限公司','AA制造工业有限公司','BB科技有限公司','CC房地产开发公司','DD物流运输有限公司','EE环保工程有限公司'];const codes=['9146**********'+String(i).padStart(4,'0')];for(let i=1;i<=40;i++){const m=Math.floor(Math.random()*6)+1;const day=Math.floor(Math.random()*28)+1;const repaired=i%3===0;d.push({seq:i,name:names[i%8],creditCode:'9146**********'+String(i).padStart(4,'0'),type:types[i%4],date:`2025-${String(m).padStart(2,'0')}-${String(day).padStart(2,'0')}`,repaired:repaired});}return d;}
