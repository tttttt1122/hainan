window.CreditPage = class CreditPage {
    ...
    bindEvents() {
        this.container.addEventListener('click', (e) => {
            const triggers = [
                ...
                ['data-credit-subject', 'openCreditSubjectModal'],
            ];
    ...
    switchDataflowTab(tab) {
        ...
        if (tab === 'category') {
            setTimeout(() => {
                if (this.charts.creditPushPie) this.charts.creditPushPie.resize();
                if (this.charts.creditReceivePie) this.charts.creditReceivePie.resize();
            }, 100);
        }
    }

    renderDataflowFlowChart() {
        const container = document.getElementById('credit-dataflow-flow-chart');
        if (!container) return;
        container.innerHTML = `
            <div class="dataflow-image-container">
                <img src="信用-数据流转.png" alt="数据流转" class="dataflow-flow-image">
            </div>
        `;
    }
    ...
    openModal(type) {
        ...
        overlay.innerHTML = `${this.renderModalContent(type)}`;
    ...
    renderModalContent(type) {
        switch (type) {
            case 'credit_b_above':
                return this.renderCreditBAboveModal();
            case 'credit_b_ratio':
                return this.renderCreditBRatioModal();
            case 'credit_violation_rate':
                return this.renderCreditViolationRateModal();
            default:
                return '';
        }
    }
    ...
    generateCreditBAboveTableRows() {
        const rows = [];
        for (let i = 1; i <= 15; i++) {
            const codes = ['91460000MA5T', '91460000MA5U', '91460000MA5V', '91460000MA5W'];
            const names = ['海南XX科技有限公司', '海南XX贸易有限公司', '海南XX投资有限公司', '海南XX服务有限公司'];
            const levels = ['A级', 'B级', 'A级', 'B级'];
            const industries = ['科技', '贸易', '投资', '服务'];
            rows.push(`<tr><td>${i}</td><td>${names[i % 4]}</td><td>${codes[i % 4]}${String(i).padStart(4, '0')}</td><td>${levels[i % 4]}</td><td>${industries[i % 4]}</td><td>2026-0${(i % 6) + 1}-${String(i % 28 + 1).padStart(2, '0')}</td><td>2027-0${(i % 6) + 1}-${String(i % 28 + 1).padStart(2, '0')}</td></tr>`);
        }
        return rows.join('');
    }
    ...
    renderCreditBRatioModal() {
        return `
            <div class="indicator-modal">
                <div class="indicator-modal-header">
                    <span class="indicator-modal-title">信用等级B级及以上市场主体占比分析</span>
                    <button class="indicator-modal-close">×</button>
                </div>
                <div class="indicator-modal-stats">
                    <div class="indicator-stat-card">
                    ...
                    <div class="indicator-stat-card">
                        <span class="indicator-stat-label">B级及以上市场主体占比</span>
                        <span class="indicator-stat-value">90%</span>
                    </div>
                    <div class="indicator-stat-card">
                        <span class="indicator-stat-label">同比增长</span>
                        <span class="indicator-stat-value" style="color:#34c759;">↑ 5%</span>
                    </div>
                </div>
                <div class="indicator-modal-chart" style="grid-template-columns: 1fr 1fr;">
                    <div class="indicator-chart-item">
                        <span class="indicator-chart-title">B级及以上市场主体占比趋势</span>
                        <div id="modal-b-ratio-line" class="indicator-chart-container"></div>
                    </div>
                    <div class="indicator-chart-item">
                        <span class="indicator-chart-title">各等级市场主体占比</span>
                        <div id="modal-b-ratio-pie" class="indicator-chart-container"></div>
                    </div>
                </div>
                <div class="indicator-modal-content">
                    <table class="indicator-modal-table">
                        ...
                        <tbody>
                            <tr><td>1</td><td>A级</td><td>210.04万户</td><td>30%</td><td>↑ 8%</td></tr>
                            <tr><td>2</td><td>B级</td><td>420.07万户</td><td>60%</td><td>↑ 3%</td></tr>
                            <tr><td>3</td><td>C级</td><td>56.01万户</td><td>8%</td><td>↓ 2%</td></tr>
                            <tr><td>4</td><td>D级</td><td>14.00万户</td><td>2%</td><td>↓ 1%</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            <div class="indicator-modal">
                ...
                <div class="indicator-modal-stats">
                    ...
                    <div class="indicator-stat-card">
                        ...
                        <span class="indicator-stat-value">80%</span>
                    </div>
                    <div class="indicator-stat-card">
                        <span class="indicator-stat-label">同比增长</span>
                        <span class="indicator-stat-value" style="color:#34c759;">↑ 3.5%</span>
                    </div>
                </div>
                <div class="indicator-modal-chart" style="grid-template-columns: 1fr 1fr;">
                    <div class="indicator-chart-item">
                        <span class="indicator-chart-title">合规经营比例趋势</span>
                        <div id="modal-violation-rate-line" class="indicator-chart-container"></div>
                    </div>
                    <div class="indicator-chart-item">
                        <span class="indicator-chart-title">违规类型分布</span>
                        <div id="modal-violation-rate-pie" class="indicator-chart-container"></div>
                    </div>
                </div>
                <div class="indicator-modal-content">
                    <table class="indicator-modal-table">
                        ...
                        <tbody>
                            <tr><td>1</td><td>一般违规</td><td>98.02万户</td><td>70%</td><td>95%</td></tr>
                            <tr><td>2</td><td>严重违规</td><td>28.00万户</td><td>20%</td><td>88%</td></tr>
                            <tr><td>3</td><td>重大违规</td><td>14.00万户</td><td>10%</td><td>82%</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
    ...
    initModalCharts(type) {
        setTimeout(() => {
            if (type === 'credit_b_above') {
                const barChart = echarts.init(document.getElementById('modal-b-level-bar'));
                barChart.setOption({
                    grid: { left: '10%', right: '5%', top: '5%', bottom: '15%' },
                    xAxis: { type: 'category', data: ['A级', 'B级', 'C级', 'D级'], axisLabel: { color: '#fff', fontSize: 12 } },
                    yAxis: { type: 'value', axisLabel: { color: '#fff', fontSize: 12 }, splitLine: { lineStyle: { color: 'rgba(0,212,255,0.1)' } } },
                    series: [{
                        type: 'bar',
                        data: [210, 420, 56, 14],
                ...
                const pieChart = echarts.init(document.getElementById('modal-b-level-pie'));
                pieChart.setOption({
                    series: [{
                        radius: ['40%', '70%'],
                        type: 'pie',
                        data: [
                            { value: 30, name: '企业法人', itemStyle: { color: '#4a9eff' } },
                            { value: 25, name: '个体工商户', itemStyle: { color: '#00c853' } },
                            { value: 20, name: '社会组织', itemStyle: { color: '#ffcc00' } },
                            { value: 25, name: '其他', itemStyle: { color: '#ff7f50' } }
                        ],
                        label: { show: true, color: '#fff', fontSize: 11, formatter: '{b}\n{d}%' },
                        labelLine: { show: true, lineStyle: { color: 'rgba(0,212,255,0.5)' } }
                    }]
                });
            } else if (type === 'credit_b_ratio') {
                const lineChart = echarts.init(document.getElementById('modal-b-ratio-line'));
                lineChart.setOption({
                    grid: { left: '10%', right: '5%', top: '5%', bottom: '15%' },
                    xAxis: { type: 'category', data: ['1月', '2月', '3月', '4月', '5月', '6月', '7月'], axisLabel: { color: '#fff', fontSize: 11 } },
                    yAxis: { type: 'value', min: 80, axisLabel: { color: '#fff', fontSize: 11 }, splitLine: { lineStyle: { color: 'rgba(0,212,255,0.1)' } } },
                    series: [{
                        type: 'line',
                        data: [85, 86, 87, 88, 89, 89.5, 90],
                        smooth: true,
                        symbol: 'circle',
                        symbolSize: 6,
                ...
                const pieChart = echarts.init(document.getElementById('modal-b-ratio-pie'));
                pieChart.setOption({
                    series: [{
                        radius: ['40%', '70%'],
                        type: 'pie',
                        data: [
                            { value: 30, name: 'A级', itemStyle: { color: '#34c759' } },
                            { value: 60, name: 'B级', itemStyle: { color: '#4a9eff' } },
                            { value: 8, name: 'C级', itemStyle: { color: '#ffcc00' } },
                            { value: 2, name: 'D级', itemStyle: { color: '#ff3b30' } }
                        ],
                        label: { show: true, color: '#fff', fontSize: 11, formatter: '{b}\n{d}%' },
                        labelLine: { show: true, lineStyle: { color: 'rgba(0,212,255,0.5)' } }
                    }]
                });
            } else if (type === 'credit_violation_rate') {
                const lineChart = echarts.init(document.getElementById('modal-violation-rate-line'));
                lineChart.setOption({
                    grid: { left: '10%', right: '5%', top: '5%', bottom: '15%' },
                    xAxis: { type: 'category', data: ['1月', '2月', '3月', '4月', '5月', '6月', '7月'], axisLabel: { color: '#fff', fontSize: 11 } },
                    yAxis: { type: 'value', min: 70, axisLabel: { color: '#fff', fontSize: 11 }, splitLine: { lineStyle: { color: 'rgba(0,212,255,0.1)' } } },
                    series: [{
                        type: 'line',
                        data: [75, 76, 77, 78, 79, 79.5, 80],
                        smooth: true,
                        symbol: 'circle',
                        symbolSize: 6,
                        lineStyle: { color: '#34c759', width: 2 },
                        itemStyle: { color: '#34c759' },
                        areaStyle: { color: 'rgba(52,199,89,0.15)' }
                    }]
                });
                
                const pieChart = echarts.init(document.getElementById('modal-violation-rate-pie'));
                pieChart.setOption({
                    series: [{
                        radius: ['40%', '70%'],
                        type: 'pie',
                        data: [
                            { value: 70, name: '一般违规', itemStyle: { color: '#ffcc00' } },
                            { value: 20, name: '严重违规', itemStyle: { color: '#ff7f50' } },
                            { value: 10, name: '重大违规', itemStyle: { color: '#ff3b30' } }
                        ],
                        label: { show: true, color: '#fff', fontSize: 11, formatter: '{b}\n{d}%' },
                        labelLine: { show: true, lineStyle: { color: 'rgba(0,212,255,0.5)' } }
                    }]
        ...
        this.container.innerHTML = `
            <div class="page-container credit-layout animate-slide-in">
                <div class="credit-content">
                    <div class="col-left">
                        <div class="card-section credit-subject-section">
                            <div class="card-title">信用主体情况</div>
                            <div class="credit-subject-content">
                                <div class="credit-subject-stats">
                                    <div class="home-credit-stat-card-vertical" onclick="window.creditPage.openModal('credit_b_above')" style="cursor: pointer;">
                                        ...
                                        <span class="home-credit-stat-rate" onclick="window.creditPage.openModal('credit_b_ratio'); event.stopPropagation();" style="cursor: pointer;">占比90%</span>
                                    </div>
                                    <div class="home-credit-stat-card-vertical" onclick="window.creditPage.openModal('credit_violation_rate')" style="cursor: pointer;">
                                ...
                                <div class="credit-subject-charts">
                                    <div class="credit-subject-chart-item">
                    ...
                    <div class="col-center">
                        ...
                        <div class="credit-stat-card" data-credit-subject="true">
                                <span class="credit-stat-value">500</span>
                                <span class="credit-stat-label">信用主体数</span>
                            </div>
                    ...
                    <div class="col-right">
                        ...
                        <div class="card-section credit-dataflow dataflow-card">
                            ...
                            <div id="dataflow-platform" class="dataflow-content">
                                <div class="dataflow-flow-chart" id="credit-dataflow-flow-chart"></div>
                            </div>
                            <div id="dataflow-category" class="dataflow-content dataflow-content-hidden">
                                <div class="dataflow-stats">
                                    <div class="dataflow-stat-item" onclick="event.stopPropagation();window.creditPage.openPushDataModal()" style="cursor: pointer;">
                                        <div class="dataflow-label-row">
                                            <span class="dataflow-label">推送数据</span>
                                            <div class="dataflow-value-box">
                                                <span class="dataflow-value">8.2</span>
                                                <span class="dataflow-unit">万条</span>
                                            </div>
                                        </div>
                                        <div id="credit-push-pie" class="chart-container pie-container mini-pie"></div>
                                    </div>
                                    <div class="dataflow-stat-item" onclick="event.stopPropagation();window.creditPage.openReceiveDataModal()" style="cursor: pointer;">
                                        <div class="dataflow-label-row">
                                            <span class="dataflow-label">收到数据</span>
                                            <div class="dataflow-value-box">
                                                <span class="dataflow-value">6.5</span>
                                                <span class="dataflow-unit">万条</span>
                                            </div>
                                        </div>
                                        <div id="credit-receive-pie" class="chart-container pie-container mini-pie"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    initCharts() {
        setTimeout(() => {
            ...
            this.initCreditPushPieChart();
            this.initCreditReceivePieChart();
            this.renderDataflowFlowChart();
        }, 300);
    }

    initCreditPushPieChart() {
        const dom = document.getElementById('credit-push-pie');
        if (!dom) return;
        dom.style.height = '80px';
        this.charts.creditPushPie = echarts.init(dom);
        this.charts.creditPushPie.setOption({
            ...
            series: [{
                ...
                data: [{ value: 100, name: '信用数据', itemStyle: { color: '#af52de' } }]
            }]
        });
        window.addEventListener('resize', () => chart.resize());
    }

    initCreditReceivePieChart() {
        const dom = document.getElementById('credit-receive-pie');
        if (!dom) return;
        dom.style.height = '80px';
        this.charts.creditReceivePie = echarts.init(dom);
        this.charts.creditReceivePie.setOption({
            tooltip: { trigger: 'item', formatter: '{b}: {c}% ({d}%)' },
            series: [{
                type: 'pie',
                radius: ['50%', '75%'],
                ...
                labelLine: { show: false },
                data: [
                    { value: 40, name: '审批办件', itemStyle: { color: '#00d4ff' } },
                    { value: 35, name: '检查结果', itemStyle: { color: '#34c759' } },
                    { value: 25, name: '处罚结果', itemStyle: { color: '#ff9500' } }
                ]
            }]
        });
        window.addEventListener('resize', () => chart.resize());
    ...
    initCreditPie() {
        const dom = document.getElementById('creditLevelPie');
        if (!dom) return;
        ...
        const option = {
            ...
            series: [{
                ...
                data: [
                    { value: 45, name: 'A级', itemStyle: { color: '#34c759' } },
                    { value: 35, name: 'B级', itemStyle: { color: '#4a9eff' } },
                    { value: 15, name: 'C级', itemStyle: { color: '#ffaa00' } },
                    { value: 5,  name: 'D级', itemStyle: { color: '#ff6b6b' } }
                ]
            }]
        };
    ...
    initSubjectPie() {
        ...
        const option = {
            ...
            series: [{
                ...
                label: { show: true, color: '#fff', fontSize: 12, formatter: (p) => `${p.value}\n${p.percent}%` },
                labelLine: { show: true, length: 8, length2: 6, lineStyle: { color: 'rgba(0,212,255,0.5)' } }
            }]
        };
        chart.setOption(option);
        window.addEventListener('resize', () => chart.resize());
    ...
    openPushDataModal() {
        ...
        this.modalData = Array.from({ length: 30 }, (_, i) => ({
            ...
            name: '信用数据',
            dept: ['省信用办', '省市场监管局', '省住建厅', '省交通厅'][i % 4],
        ...
        overlay.innerHTML = `
            <div class="indicator-modal">
                ...
                <div class="indicator-modal-stats">
                    <div class="indicator-stat-card"><span class="indicator-stat-label">推送数据总量</span><span class="indicator-stat-value">8.2万条</span></div>
                    <div class="indicator-stat-card"><span class="indicator-stat-label">信用数据</span><span class="indicator-stat-value">100%</span></div>
                    <div class="indicator-stat-card"><span class="indicator-stat-label">覆盖部门</span><span class="indicator-stat-value">24个</span></div>
                </div>
                <div class="indicator-modal-chart" style="display:flex;gap:10px;">
                    <div class="indicator-chart-item" style="flex:1;"><span class="indicator-chart-title">推送数据类型分布</span><div id="credit-modal-push-pie" class="indicator-chart-container"></div></div>
                    <div class="indicator-chart-item" style="flex:1;"><span class="indicator-chart-title">各部门推送数据量</span><div id="credit-modal-push-bar" class="indicator-chart-container"></div></div>
                </div>
                <div class="indicator-modal-content">
                    <table class="indicator-modal-table"><thead><tr><th>序号</th><th>推送编号</th><th>数据类型</th><th>推送部门</th><th>推送数量</th><th>推送日期</th></tr></thead>
                    <tbody>${this.modalData.slice(0, 5).map((d, i) => `<tr><td>${i + 1}</td><td>${d.id}</td><td>${d.name}</td><td>${d.dept}</td><td>${d.count}</td><td>${d.date}</td></tr>`).join('')}</tbody></table>
                </div>
                <div class="indicator-modal-pagination">
                    <button class="indicator-pagination-btn" id="generic-modal-prev" disabled>上一页</button>
                    <span class="indicator-pagination-info">第 1 / ${Math.ceil(this.modalData.length / 5)} 页</span>
                    <button class="indicator-pagination-btn" id="generic-modal-next">下一页</button>
                </div>
            </div>
        `;
        document.body.appendChild(overlay);
        this.bindCreditModalEvents(overlay);
        setTimeout(() => this.initCreditPushModalCharts(), 80);
    }
    ...
    openReceiveDataModal() {
        ...
        this.modalData = Array.from({ length: 30 }, (_, i) => ({
            ...
            name: ['审批办件', '检查结果', '处罚结果'][i % 3],
            dept: ['省审批局', '省市场监管局', '省综合执法局'][i % 3],
            count: Math.floor(Math.random() * 500) + 100,
            date: '2026-07-' + String(i % 28 + 1).padStart(2, '0')
        }));
        this.currentPage = 1;
        overlay.innerHTML = `
            <div class="indicator-modal">
                ...
                <div class="indicator-modal-stats">
                    <div class="indicator-stat-card"><span class="indicator-stat-label">收到数据总量</span><span class="indicator-stat-value">6.5万条</span></div>
                    <div class="indicator-stat-card"><span class="indicator-stat-label">审批办件</span><span class="indicator-stat-value">40%</span></div>
                    <div class="indicator-stat-card"><span class="indicator-stat-label">检查结果</span><span class="indicator-stat-value">35%</span></div>
                    <div class="indicator-stat-card"><span class="indicator-stat-label">处罚结果</span><span class="indicator-stat-value">25%</span></div>
                    <div class="indicator-stat-card"><span class="indicator-stat-label">覆盖部门</span><span class="indicator-stat-value">20个</span></div>
                </div>
                <div class="indicator-modal-chart" style="display:flex;gap:10px;">
                    <div class="indicator-chart-item" style="flex:1;"><span class="indicator-chart-title">收到数据类型分布</span><div id="credit-modal-receive-pie" class="indicator-chart-container"></div></div>
                    <div class="indicator-chart-item" style="flex:1;"><span class="indicator-chart-title">各来源收到数据量</span><div id="credit-modal-receive-bar" class="indicator-chart-container"></div></div>
                </div>
                <div class="indicator-modal-content">
        ...
        document.body.appendChild(overlay);
        this.bindCreditModalEvents(overlay);
        setTimeout(() => this.initCreditReceiveModalCharts(), 80);
    }

    bindCreditModalEvents(overlay) {
        ...
        prevBtn?.addEventListener('click', () => {
            if (this.currentPage > 1) { this.currentPage--; this.updateCreditModal(overlay); }
        });
        nextBtn?.addEventListener('click', () => {
            if (this.currentPage < Math.ceil(this.modalData.length / 5)) { this.currentPage++; this.updateCreditModal(overlay); }
        });
    }

    updateCreditModal(overlay) {
        const start = (this.currentPage - 1) * 5;
        const end = start + 5;
        const pageData = this.modalData.slice(start, end);
        const totalPages = Math.ceil(this.modalData.length / 5);
    ...
    initCreditPushModalCharts() {
        const pieChart = echarts.init(document.getElementById('credit-modal-push-pie'));
        pieChart.setOption({
            tooltip: { trigger: 'item', formatter: '{b}: {c}% ({d}%)' },
            series: [{
        ...
        const barChart = echarts.init(document.getElementById('credit-modal-push-bar'));
        barChart.setOption({
            tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
            grid: { left: '10%', right: '5%', top: '15%', bottom: '10%' },
            xAxis: { type: 'category', data: ['信用办', '市场监管', '住建', '交通'], axisLabel: { color: '#fff' } },
            yAxis: { type: 'value', axisLabel: { color: '#fff' } },
            series: [{ type: 'bar', data: [4000, 3200, 2200, 1800], itemStyle: { color: '#af52de' } }]
        });
    }

    initCreditReceiveModalCharts() {
        const pieChart = echarts.init(document.getElementById('credit-modal-receive-pie'));
        pieChart.setOption({
            tooltip: { trigger: 'item', formatter: '{b}: {c}% ({d}%)' },
            series: [{
                type: 'pie', radius: ['45%', '70%'], center: ['50%', '50%'],
                data: [{ value: 40, name: '审批办件', itemStyle: { color: '#00d4ff' } }, { value: 35, name: '检查结果', itemStyle: { color: '#34c759' } }, { value: 25, name: '处罚结果', itemStyle: { color: '#ff9500' } }]
            }]
        });
        const barChart = echarts.init(document.getElementById('credit-modal-receive-bar'));
        barChart.setOption({
            tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
            grid: { left: '10%', right: '5%', top: '15%', bottom: '10%' },
            xAxis: { type: 'category', data: ['审批局', '市场监管', '综合执法'], axisLabel: { color: '#fff' } },
            yAxis: { type: 'value', axisLabel: { color: '#fff' } },
            series: [{ type: 'bar', data: [3200, 2500, 1800], itemStyle: { color: '#00d4ff' } }]
        });
    }

    openPushApprovalModal() {
        const months = ['2025-06','2025-07','2025-08','2025-09','2025-10','2025-11','2025-12','2026-01','2026-02','2026-03','2026-04','2026-05'];
        const trendData = [5680, 6120, 6540, 5980, 6890, 7230, 7650, 6540, 6230, 6980, 7560, 7920];
        const sourceData = [
            { value: 28700, name: '信用等级信息', itemStyle: { color: '#4a9eff' } },
            { value: 20500, name: '失信主体名单', itemStyle: { color: '#ff7f50' } },
            { value: 16400, name: '信用修复信息', itemStyle: { color: '#00c853' } },
            { value: 10250, name: '红黑名单', itemStyle: { color: '#ffcc00' } },
            { value: 6150, name: '其他', itemStyle: { color: '#af52de' } }
        ];
        const types = ['信用等级信息', '失信主体名单', '信用修复信息', '红黑名单', '其他'];
        const fields = ['市场监管', '金融', '税务', '社保', '环保', '交通'];
        const sendDepts = ['海南省信用办', '海口市信用办', '三亚市信用办', '儋州市信用办'];
        const recvDepts = ['海南省审批服务局', '海口市审批服务局', '三亚市审批服务局'];
        const statuses = ['已接收', '未接收', '处理中'];
        const rows = [];
        for (let i = 1; i <= 15; i++) {
            const id = `XS-SP${String(i).padStart(6, '0')}`;
            const isEnterprise = Math.random() > 0.3;
            const name = isEnterprise ? `海南${['科技','实业','商贸','物流','旅游','农业'][i%6]}有限公司` : `张${['三','伟','强','明','辉','刚'][i%6]}`;
            const credit = isEnterprise ? `${Math.floor(Math.random()*9)+1}${String(Math.floor(Math.random()*1000000000000)).padStart(12, '0')}` : `${Math.floor(Math.random()*900+100)}${String(Math.floor(Math.random()*100000000)).padStart(8, '0')}`;
            rows.push([id, name, credit, types[i%5], fields[i%6], sendDepts[i%4], recvDepts[i%3], `${2025+Math.floor(i/12)}-${String((i%12)+1).padStart(2,'0')}-${String(Math.floor(Math.random()*28)+1).padStart(2,'0')}`, statuses[Math.floor(Math.random()*3)]]);
        }
        this._openPushModal({
            title: '信用推送审批数据量分析',
            kpis: [
                { label: '累计推送审批信用数据', value: '8.2', suffix: '万条', highlight: 'total' },
                { label: '本月推送', value: '3,568', suffix: '条' },
                { label: '已接收', value: '7.9', suffix: '万条' },
                { label: '接收率', value: '96.3', suffix: '%' }
            ],
            chart1Title: '近12个月推送审批信用数据趋势',
            chart1Type: 'line',
            chart1Data: { x: months, y: trendData },
            chart2Title: '推送信用数据类型分布',
            chart2Type: 'pie',
            chart2Data: sourceData,
            tableTitle: '推送审批信用数据明细列表',
            table: {
                header: ['推送信用数据编号', '企业/个人名称', '统一社会信用代码/身份证号', '信用数据类型', '数据归属领域', '推送部门', '接收部门', '推送时间', '接收状态'],
                rows: rows
            }
        });
    }
    ...
    openPushSupervisionModal() {
        const months = ['2025-06','2025-07','2025-08','2025-09','2025-10','2025-11','2025-12','2026-01','2026-02','2026-03','2026-04','2026-05'];
        const trendData = [4250, 4680, 5120, 4780, 5450, 5820, 6150, 5230, 4980, 5560, 6020, 6430];
        const sourceData = [
            { value: 22750, name: '信用等级信息', itemStyle: { color: '#4a9eff' } },
            { value: 16250, name: '失信主体名单', itemStyle: { color: '#ff7f50' } },
            { value: 13000, name: '信用修复信息', itemStyle: { color: '#00c853' } },
            { value: 8125, name: '红黑名单', itemStyle: { color: '#ffcc00' } },
            { value: 4875, name: '其他', itemStyle: { color: '#af52de' } }
        ];
        const types = ['信用等级信息', '失信主体名单', '信用修复信息', '红黑名单', '其他'];
        const fields = ['市场监管', '生态环境', '交通运输', '卫生健康', '应急管理', '税务'];
        const sendDepts = ['海南省信用办', '海口市信用办', '三亚市信用办', '文昌市信用办'];
        const recvDepts = ['海南省市场监管局', '海南省生态环境厅', '海南省交通厅'];
        const statuses = ['已接收', '未接收', '处理中'];
        const rows = [];
        for (let i = 1; i <= 15; i++) {
            const id = `XS-JG${String(i).padStart(6, '0')}`;
            const isEnterprise = Math.random() > 0.3;
            const name = isEnterprise ? `海南${['环保','交通','食品','医药','建筑','物流'][i%6]}有限公司` : `李${['娜','静','芳','燕','敏','丽'][i%6]}`;
            const credit = isEnterprise ? `${Math.floor(Math.random()*9)+1}${String(Math.floor(Math.random()*1000000000000)).padStart(12, '0')}` : `${Math.floor(Math.random()*900+100)}${String(Math.floor(Math.random()*100000000)).padStart(8, '0')}`;
            rows.push([id, name, credit, types[i%5], fields[i%6], sendDepts[i%4], recvDepts[i%3], `${2025+Math.floor(i/12)}-${String((i%12)+1).padStart(2,'0')}-${String(Math.floor(Math.random()*28)+1).padStart(2,'0')}`, statuses[Math.floor(Math.random()*3)]]);
        }
        this._openPushModal({
            title: '信用推送监管数据量分析',
            kpis: [
                { label: '累计推送监管信用数据', value: '6.5', suffix: '万条', highlight: 'total' },
                { label: '本月推送', value: '2,897', suffix: '条' },
                { label: '已接收', value: '6.3', suffix: '万条' },
                { label: '接收率', value: '96.9', suffix: '%' }
            ],
            chart1Title: '近12个月推送监管信用数据趋势',
            chart1Type: 'line',
            chart1Data: { x: months, y: trendData },
            chart2Title: '推送信用数据类型分布',
            chart2Type: 'pie',
            chart2Data: sourceData,
            tableTitle: '推送监管信用数据明细列表',
            table: {
                header: ['推送信用数据编号', '企业/个人名称', '统一社会信用代码/身份证号', '信用数据类型', '数据归属领域', '推送部门', '接收部门', '推送时间', '接收状态'],
                rows: rows
            }
    ...
    openPushLawModal() {
        const months = ['2025-06','2025-07','2025-08','2025-09','2025-10','2025-11','2025-12','2026-01','2026-02','2026-03','2026-04','2026-05'];
        const trendData = [3120, 3450, 3820, 3560, 4020, 4350, 4680, 3920, 3680, 4150, 4560, 4820];
        const sourceData = [
            { value: 16800, name: '信用等级信息', itemStyle: { color: '#4a9eff' } },
            { value: 12000, name: '失信主体名单', itemStyle: { color: '#ff7f50' } },
            { value: 9600, name: '信用修复信息', itemStyle: { color: '#00c853' } },
            { value: 6000, name: '红黑名单', itemStyle: { color: '#ffcc00' } },
            { value: 3600, name: '其他', itemStyle: { color: '#af52de' } }
        ];
        const types = ['信用等级信息', '失信主体名单', '信用修复信息', '红黑名单', '其他'];
        const fields = ['市场监管', '税务', '生态环境', '交通运输', '卫生健康', '文化旅游'];
        const sendDepts = ['海南省信用办', '儋州市信用办', '琼海市信用办', '万宁市信用办'];
        const recvDepts = ['海南省综合执法局', '海口市综合执法局', '三亚市综合执法局'];
        const statuses = ['已接收', '未接收', '处理中'];
        const rows = [];
        for (let i = 1; i <= 15; i++) {
            const id = `XS-ZF${String(i).padStart(6, '0')}`;
            const isEnterprise = Math.random() > 0.3;
            const name = isEnterprise ? `海南${['商贸','餐饮','娱乐','建材','电子','食品'][i%6]}有限公司` : `王${['勇','军','豪','杰','鹏','涛'][i%6]}`;
            const credit = isEnterprise ? `${Math.floor(Math.random()*9)+1}${String(Math.floor(Math.random()*1000000000000)).padStart(12, '0')}` : `${Math.floor(Math.random()*900+100)}${String(Math.floor(Math.random()*100000000)).padStart(8, '0')}`;
            rows.push([id, name, credit, types[i%5], fields[i%6], sendDepts[i%4], recvDepts[i%3], `${2025+Math.floor(i/12)}-${String((i%12)+1).padStart(2,'0')}-${String(Math.floor(Math.random()*28)+1).padStart(2,'0')}`, statuses[Math.floor(Math.random()*3)]]);
        }
        this._openPushModal({
            title: '信用推送执法数据量分析',
            kpis: [
                { label: '累计推送执法信用数据', value: '4.8', suffix: '万条', highlight: 'total' },
                { label: '本月推送', value: '1,923', suffix: '条' },
                { label: '已接收', value: '4.5', suffix: '万条' },
                { label: '接收率', value: '93.8', suffix: '%' }
            ],
            ...
            <div class="trend-pager-bar" style="margin-top:6px;">
                ...
                <div class="trend-pager-btns">
                    <button class="trend-pager-btn ${cur<=1?'disabled':''}"
                            onclick="window.creditPage._switchPushPage(-1)">上一页</button>
                    <span class="trend-pager-num">第 <b>${cur}</b> / ${last} 页</span>
                    <button class="trend-pager-btn ${cur>=last?'disabled':''}"
                            onclick="window.creditPage._switchPushPage(1)">下一页</button>
                </div>
            </div>`;
        };
        ...
        <div class="indicator-modal panorama-modal" style="width:85vw;max-width:1400px;">
            ...
            <div class="trend-charts-row">
                <div class="trend-chart-box trend-chart-main">
                    ...
                    <div id="credit-push-chart-left" class="trend-chart-container"></div>
                </div>
                <div class="trend-chart-box trend-chart-pie">
                    <div class="trend-chart-title">${cfg.chart2Title}</div>
                    <div id="credit-push-chart-right" class="trend-chart-container"></div>
                </div>
            </div>
            <div class="trend-table-wrap">
                ...
                <div class="trend-table-scroller">
                    <table class="trend-data-table">
                        ...
                        <tbody id="credit-push-tbody">${renderTbody()}</tbody>
                    </table>
                </div>
                <div class="trend-pager-bar" id="credit-push-pager">${renderPager()}</div>
            </div>
        </div>`;

        document.body.appendChild(overlay);
        overlay.querySelector('.indicator-modal-close').addEventListener('click', () => this._closePushModal());
        overlay.addEventListener('click', e => { if (e.target === overlay) this._closePushModal(); });
        setTimeout(() => this._initPushCharts(), 80);
    }
    ...
    _switchPushPage(delta) {
        const next = this._panoPage + delta;
        if (next < 1 || next > this._panoPages) return;
        this._panoPage = next;
        const b = document.getElementById('credit-push-tbody');
        if (b) {
            const start = (this._panoPage - 1) * this._panoPageSize;
        ...
        const p = document.getElementById('credit-push-pager');
        if (p) {
            const t = this._panoTotal, cur = this._panoPage, last = this._panoPages;
            p.innerHTML = `
            <div class="trend-pager-bar" style="margin-top:6px;">
    ...
    _initPushCharts() {
        ...
        if (!cfg) return;
        const leftEl = document.getElementById('credit-push-chart-left');
        const rightEl = document.getElementById('credit-push-chart-right');
        if (!leftEl || !rightEl) return;

        const renderLine = (el, data) => {
        ...
        const leftChart = cfg.chart1Type === 'line' ? renderLine(leftEl, cfg.chart1Data) : null;
        const rightChart = cfg.chart2Type === 'pie' ? renderPie(rightEl, cfg.chart2Data) : null;
        window.addEventListener('resize', () => {
            try { if (leftChart) leftChart.resize(); } catch(e) {}
            try { if (rightChart) rightChart.resize(); } catch(e) {}
        });
    }
    ...
    openCreditSubjectModal() {
        ...
        const trendData = [420, 435, 448, 460, 475, 450, 455, 470, 480, 485, 492, 500];

        this._openPushModal({
            title: '信用主体总量趋势分析',
            kpis: [
                { label: '信用主体总量', value: '500', suffix: '个', highlight: 'total' },
                { label: '去年同期', value: '568', suffix: '个' },
                { label: '同比减少', value: '68', suffix: '个' },
                { label: '同比下降', value: '12', suffix: '%' }
            ],
            chart1Title: '近12个月信用主体趋势',
            chart1Type: 'line',
            chart1Data: { x: months, y: trendData },
            chart2Title: '信用等级分布',
            chart2Type: 'pie',
            chart2Data: [
                { value: 225, name: 'A级', itemStyle: { color: '#34c759' } },
                { value: 200, name: 'B级', itemStyle: { color: '#4a9eff' } },
                { value: 50,  name: 'C级', itemStyle: { color: '#ffaa00' } },
                { value: 25,  name: 'D级', itemStyle: { color: '#ff6b6b' } }
            ],
            tableTitle: '信用主体明细列表',
            table: {
